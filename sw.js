console.log("[Axis] Service Worker started");
...
async function ensureBareClient() {
    ...
    if (!bareClient || !scramjet.client) {
        ...
        console.log("[Axis] BareClient ready with", wispConfig.wispurl);
    }
    return bareClient;
}
...
function switchToServer(url, latency = null) {
    ...
    console.log(`[Axis] Switching from ${wispConfig.wispurl} to ${url}`);
...
self.addEventListener("message", ({ data }) => {
    if (data.type === "config") {
        if (data.wispurl) {
            ...
            console.log("[Axis] Received wispurl", data.wispurl);
            currentServerStartTime = Date.now();
        }
        if (data.servers && data.servers.length > 0) {
            wispConfig.servers = data.servers;
            console.log("[Axis] Received servers", data.servers.length);
...
self.addEventListener("fetch", (event) => {
    event.respondWith((async () => {
        ...
        if (isAdBlocked(event.request.url)) {
            console.log("[Axis] Blocked ad request:", event.request.url);
                ...
                for (const server of wispConfig.servers) {
                    ...
                    if (!serverH || serverH.consecutiveFailures < MAX_CONSECUTIVE_FAILURES) {
                        ...
                        if (pingResult.success) {
                            console.log(`[Axis] Auto-switching to ${server.url} due to failures on current server`);
